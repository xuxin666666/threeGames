    ...
    useEffect(() => {
        ...
        function switchPage() {
            if (!switchPage.switched) {
                var e = { intro: window.$(".content-intro"), path: window.$(".shape-wrap path"), shape: window.$("svg.shape") };
                e.shape.style.transformOrigin = "50% 0%"
                anime({ targets: e.intro, duration: 1100, easing: "easeInOutSine", translateY: "-200vh" })
                anime({ targets: e.shape, scaleY: [{ value: [.8, 1.8], duration: 550, easing: "easeInQuad" }, { value: 1, duration: 550, easing: "easeOutQuad" }] })
                anime({
                    targets: e.path, duration: 1100, easing: "easeOutQuad", d: null, complete: function (e) {
                        if (canvas) {
        ...
        function getWebGLContext(e) {
            var r, t, n = { alpha: !0, depth: !1, stencil: !1, antialias: !1, preserveDrawingBuffer: !1 }, i = e.getContext("webgl2", n), a = !!i;
            a || (i = e.getContext("webgl", n) || e.getContext("experimental-webgl", n))
            t = a ? (i.getExtension("EXT_color_buffer_float"), i.getExtension("OES_texture_float_linear")) : (r = i.getExtension("OES_texture_half_float"), i.getExtension("OES_texture_half_float_linear"))
            i.clearColor(0, 0, 0, 1);
            var o, l, u, c = a ? i.HALF_FLOAT : r.HALF_FLOAT_OES;
            u = a ? (o = getSupportedFormat(i, i.RGBA16F, i.RGBA, c), l = getSupportedFormat(i, i.RG16F, i.RG, c), getSupportedFormat(i, i.R16F, i.RED, c)) : (o = getSupportedFormat(i, i.RGBA, i.RGBA, c), l = getSupportedFormat(i, i.RGBA, i.RGBA, c), getSupportedFormat(i, i.RGBA, i.RGBA, c))
            return { gl: i, ext: { formatRGBA: o, formatRG: l, formatR: u, halfFloatTexType: c, supportLinearFiltering: t } }
        }
        ...
        isMobile() && (window.config.DYE_RESOLUTION = 512);
        if (!ext.supportLinearFiltering) {
            window.config.DYE_RESOLUTION = 512
            window.config.SHADING = !1
            window.config.BLOOM = !1
            window.config.SUNRAYS = !1
        }
            ...
            _createClass(t, [{
                key: "setKeywords", value: function (e) {
                    ...
                    if (n !== this.activeProgram) {
                        this.uniforms = getUniforms(n)
                        this.activeProgram = n
                    }
                }
            }, { key: "bind", value: function () { gl.useProgram(this.activeProgram) } }])
            return t
        }(),
            Program = function () {
                function t(e, r) {
                ...
                _createClass(t, [{ key: "bind", value: function () { gl.useProgram(this.program) } }])
                return t
            }();
        ...
        function initFramebuffers() {
            var e = getResolution(window.config.SIM_RESOLUTION),
                r = getResolution(window.config.DYE_RESOLUTION),
                t = ext.halfFloatTexType,
                n = ext.formatRGBA,
                i = ext.formatRG,
                a = ext.formatR,
                o = ext.supportLinearFiltering ? gl.LINEAR : gl.NEAREST;
            dye = null == dye ? createDoubleFBO(r.width, r.height, n.internalFormat, n.format, t, o) : resizeDoubleFBO(dye, r.width, r.height, n.internalFormat, n.format, t, o)
            velocity = null == velocity ? createDoubleFBO(e.width, e.height, i.internalFormat, i.format, t, o) : resizeDoubleFBO(velocity, e.width, e.height, i.internalFormat, i.format, t, o)
            divergence = createFBO(e.width, e.height, a.internalFormat, a.format, t, gl.NEAREST)
            curl = createFBO(e.width, e.height, a.internalFormat, a.format, t, gl.NEAREST)
            pressure = createDoubleFBO(e.width, e.height, a.internalFormat, a.format, t, gl.NEAREST)
            initBloomFramebuffers()
            initSunraysFramebuffers()
        }

        function initBloomFramebuffers() {
            var e = getResolution(window.config.BLOOM_RESOLUTION),
            ...
            for (var i = bloomFramebuffers.length = 0; i < window.config.BLOOM_ITERATIONS; i++) {
                var a = e.width >> i + 1,
                    o = e.height >> i + 1;
                if (a < 2 || o < 2) break;
        ...
        function initSunraysFramebuffers() {
            var e = getResolution(window.config.SUNRAYS_RESOLUTION),
                r = ext.halfFloatTexType,
                t = ext.formatR,
                n = ext.supportLinearFiltering ? gl.LINEAR : gl.NEAREST;
        ...
        function createFBO(e, r, t, n, i, a) {
            ...
            return {
                texture: o, fbo: l, width: e, height: r, texelSizeX: 1 / e, texelSizeY: 1 / r, attach: function (e) {
        ...
        function createDoubleFBO(e, r, t, n, i, a) {
            ...
            return {
                width: e, height: r, texelSizeX: o.texelSizeX, texelSizeY: o.texelSizeY, get read() { return o }, set read(e) { o = e }, get write() { return l }, set write(e) { l = e }, swap: function () {
                    var e = o;
                    o = l
                    l = e
                }
        ...
        function updateKeywords() {
            var e = [];
            window.config.SHADING && e.push("SHADING")
            window.config.BLOOM && e.push("BLOOM")
            window.config.SUNRAYS && e.push("SUNRAYS")
        ...
        window.addEventListener(window.visibilityChangeEvent, initBackground)
        initBackground()
        ...
        function updateColors(e) {
            if (window.config.COLORFUL && 1 <= (colorUpdateTimer += e * window.config.COLOR_UPDATE_SPEED)) {
        ...
        function step(e) {
            ...
            gl.uniform1f(clearProgram.uniforms.value, window.config.PRESSURE)
            blit(pressure.write.fbo)
            pressure.swap()
            pressureProgram.bind()
            gl.uniform2f(pressureProgram.uniforms.texelSize, velocity.texelSizeX, velocity.texelSizeY)
            gl.uniform1i(pressureProgram.uniforms.uDivergence, divergence.attach(0));
            for (var r = 0; r < window.config.PRESSURE_ITERATIONS; r++) {
        ...
        function render(e) {
            window.config.BLOOM && applyBloom(dye.read, bloom)
            if (window.config.SUNRAYS) {
                applySunrays(dye.read, dye.write, sunrays)
                blur(sunrays, sunraysTemp, 1)
            }
            if (null != e && window.config.TRANSPARENT) {
                gl.disable(gl.BLEND)
            } else {
            ...
            window.config.TRANSPARENT || drawColor(n, normalizeColor(window.config.BACK_COLOR))
            null == e && window.config.TRANSPARENT && drawCheckerboard(n)
            drawDisplay(n, r, t)
        }
        ...
        function drawDisplay(e, r, t) {
            ...
            window.config.SHADING && gl.uniform2f(displayMaterial.uniforms.texelSize, 1 / r, 1 / t)
        ...
        function applyBloom(e, r) {
            if (!(bloomFramebuffers.length < 2)) {
                ...
                var n = window.config.BLOOM_THRESHOLD * window.config.BLOOM_SOFT_KNEE + 1e-4,
                    i = window.config.BLOOM_THRESHOLD - n,
                    a = 2 * n,
                    o = .25 / n;
        ...
        function splatPointer(e) {
            var r = e.deltaX * window.config.SPLAT_FORCE,
                t = e.deltaY * window.config.SPLAT_FORCE;
        ...
        function splat(e, r, t, n, i) {
            ...
            gl.uniform1f(splatProgram.uniforms.radius, correctRadius(window.config.SPLAT_RADIUS / 100))
            blit(velocity.write.fbo)
            velocity.swap()
            gl.viewport(0, 0, dye.width, dye.height)
        ...
        document.addEventListener("touchend", touchend2);
        ...
        function keydown(e){
            "KeyP" === e.code && (window.config.PAUSED = !window.config.PAUSED)
        ...
        function touchstart(e){
            if (!switchPage || !switchPage.switched) {
                e.preventDefault(); for (var r = e.targetTouches; r.length >= pointers.length;) pointers.push(new pointerPrototype()); for (var t = 0; t < r.length; t++) {
        ...
        function touchend2(e){
            if (!switchPage || !switchPage.switched)
            for (var t = e.changedTouches, r = function (r) {
                var e = pointers.find(function (e) { return e.id === t[r].identifier }); if (null == e) return "continue";
                updatePointerUpData(e)
            }, n = 0; n < t.length; n++) r(n)
        }

        return () => {
            window.removeEventListener(window.visibilityChangeEvent, initBackground)
            ...
            window.removeEventListener('touchend', touchend)
            window.removeEventListener('touchend2', touchend)
        }
    }, [])

    return (
        <div className="IslandAndStoreMain" style={{zIndex: zIndex || 0}}>
            <div className="content-inner content content-intro">
